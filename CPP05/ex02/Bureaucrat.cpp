...
{
	/* if (_grade > form.execGrade())
	{
		std::cerr << _name << " couldn't execute " << form.getName() << " because " << _grade << " > " << form.execGrade() << std::endl;
		return;
	}
	else if (!form.isSigned())
	{
		std::cerr << _name << " couldn't execute " << form.getName() << " because it is not signed." << std::endl;
		return;
	} */
	try {
		form.execute(*this);
